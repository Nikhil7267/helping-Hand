...
class signup_volunteer(View):
    ...
    def post(self,request):
        ...
        if form1.is_valid() & form2.is_valid():
            ...
            try:
              user = User.objects.create_user(first_name=fn, last_name=ln, username=us, email=em, password=pwd) 
              Volunteer.objects.create(user = user, contact=contact, userpic=userpic, idpic=idpic, address=address, aboutme=aboutme ,
              status='pending')
...
def donationdelivered_admin(request):
    if not request.user.is_authenticated:
        return redirect('/login-admin')
    donation = Donation.objects.filter(status='Donation delivered sucessfullyd')
...
class changepwd_admin(View):
     ...
     def post(self,request):
        form1=MyPasswordChangeForm(request.user,request.POST)
        if not request.user.is_authenticated:
            return redirect('/login-admin')
        ...
        try:
            if newpass == confirmpass:
              ...
              if user.check_password(old):
                  ...
                  messages.warning(request,'old Password not matcvhed')
            else:
    ...
    def post(self, request):
        ...
        try:
            if 'userpic' in request.FILES:
             ...
             messages.warning(request,'profile Updated Failed'+ str(e))
        return render(request,'profile-donor.html',locals())
...
def donationdelivered_volunteer(request):
    ...
    user=request.user
    volunteer=Volunteer.objects.get(user=user)
    donation = Donation.objects.filter(volunteer=volunteer,status="Donation delivered sucessfully")
